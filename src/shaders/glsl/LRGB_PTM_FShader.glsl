  ...
  for (int l = 0; l < NUM_DIR_LIGHTS; l++) {
    ...
    // if (dot(lDirPTM,N) >= 0.0) {
      ...
      if (renderFuncIndex == 0) { // Malzbender
        color += directionalLights[l].color * lum * (easeOutLum*kd*diffuseRGB + kRGBs*ks_flat*pow(max(nDotHPTM,0.0),alpha));
      } else if (renderFuncIndex == 1) { // Truvis
        color += directionalLights[l].color * (lum*easeOutLum*kd*diffuseRGB + kRGBs*ks_flat*pow(max(nDotHPTM,0.0),alpha));
      } else if (renderFuncIndex == 2) { // Malzbender alt
        color += directionalLights[l].color * easeOutLum*lum * (kd*diffuseRGB + kRGBs*ks_flat*pow(max(nDotHPTM,0.0),alpha));
      }
    }
...
//    if (!gl_FrontFacing) {
    ...
    // if (dot(lDir_point_PTM,N) >= 0.0) {
      ...
      if (renderFuncIndex == 0) { // Malzbender
        color += pointLights[l].color * lum_point * (easeOutLum*kd*diffuseRGB + kRGBs*ks_flat*pow(max(nDotH_point_PTM,0.0),alpha));
      } else if (renderFuncIndex == 1) { // Truvis
        color += pointLights[l].color * (lum_point*easeOutLum*kd*diffuseRGB + kRGBs*ks_flat*pow(max(nDotH_point_PTM,0.0),alpha));
      } else if (renderFuncIndex == 2) { // Malzbender alt
        color += pointLights[l].color * easeOutLum*lum_point * (kd*diffuseRGB + kRGBs*ks_flat*pow(max(nDotH_point_PTM,0.0),alpha));
      }
    }
...
//   if (!gl_FrontFacing) {
   if (dot(viewDir, faceNormal) > 0.0) { // fix b
    ...
    // if (dot(lDir_spot_PTM,N) >= 0.0) {
      ...
      if (renderFuncIndex == 0) { // Malzbender
        color += penumbraFactor * spotLights[l].color * lum_spot * (easeOutLum*kd*diffuseRGB + kRGBs*ks_flat*pow(max(nDotH_spot_PTM,0.0),alpha));
      } else if (renderFuncIndex == 1) { // Truvis
        color += penumbraFactor * spotLights[l].color * (lum_spot*easeOutLum*kd*diffuseRGB + kRGBs*ks_flat*pow(max(nDotH_spot_PTM,0.0),alpha));
      } else if (renderFuncIndex == 2) { // Malzbender alt
        color += penumbraFactor * spotLights[l].color * easeOutLum*lum_spot * (kd*diffuseRGB + kRGBs*ks_flat*pow(max(nDotH_spot_PTM,0.0),alpha));
      }
    }
